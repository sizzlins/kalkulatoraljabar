...
def plot_function(expression: str, variable: str = "x", 
				  x_min: float = -10, x_max: float = 10,
				  points: int = 100, ascii: bool = False) -> EvalResult:
	...
	if not HAS_MATPLOTLIB and not ascii:
		return EvalResult(ok=False, error="matplotlib not installed. Use ascii=True for ASCII plot.")
	...
	try:
		...
		if ascii:
			...
			if not valid_y:
				return EvalResult(ok=False, error="Cannot plot: function values out of range")
			...
			# Add axes
			x_axis_row = int((0 - y_min) / y_range * (rows - 1)) if y_min <= 0 <= y_max else -1
			y_axis_col = int((0 - x_min) / (x_max - x_min) * (cols - 1)) if x_min <= 0 <= x_max else -1
		...
		return EvalResult(ok=False, error="Plotting failed")
