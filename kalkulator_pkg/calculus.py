...
from .parser import parse_preprocessed, format_solution
	...
	try:
		...
		if variable:
			...
			if var_sym not in free_syms:
				return EvalResult(ok=False, error=f"Variable '{variable}' not found in expression")
		...
		return EvalResult(ok=False, error="Differentiation failed")
...
def integrate(expression: str, variable: Optional[str] = None) -> EvalResult:
	...
	try:
		...
		return EvalResult(ok=False, error="Integration failed")
...
def matrix_determinant(matrix_str: str) -> EvalResult:
	...
	try:
		...
		return EvalResult(ok=False, error="Determinant calculation failed")
